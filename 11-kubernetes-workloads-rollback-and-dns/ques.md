...
- Live StatefulSet verification: deploy a headless Service + 3-replica MySQL StatefulSet (`statefulset-demo/`), verify ordinal naming and strictly sequential startup, PVC-per-ordinal binding, headless-service multi-A-record DNS, and identity invariance (delete `mysql-0`, confirm it comes back as `mysql-0`) - directly contrasted against a Deployment Pod's random-hash replacement
